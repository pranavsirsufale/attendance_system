...
function AttendanceForm({ onClose }) {
  ...
  useEffect(() => {
    const fetchSessionData = async () => {
      ...
      try {
        ...
        (response.data.attendance || []).forEach((entry) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError('Failed to mark/update attendance');
    }
  };
  ...
  return (
    <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg shadow-lg z-50 max-w-2xl w-full max-h-[80vh] overflow-y-auto">
      <h2 className="text-xl font-bold mb-4 text-gray-800">Mark/Update Attendance for Session {session?.id}</h2>
      {session && (
        <div className="mb-4">
      ...
      <form onSubmit={handleSubmit}>
        {students.map((student) => (
          <div key={student.id} className="mb-4 flex items-center space-x-4">
            <span className="text-gray-800">{student.first_name} {student.last_name} ({student.roll_number})</span>
            <select
