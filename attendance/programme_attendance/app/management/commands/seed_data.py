...
class Command(BaseCommand):
    ...
    def handle(self,*args,**kwargs):
        # Program
        ballb = Program.objects.update_or_create(name = 'BALLB 5 Yr', duration_years = 5)[0]
        llb = Program.objects.update_or_create(name = 'LLB 3 Yr' , duration_years = 3)[0]

        # Section 
        Section.objects.update_or_create(program = ballb , name = 'Section A'  , year = 1)
        Section.objects.update_or_create(program = ballb , name = 'Section B' , year = 1)

        Section.objects.update_or_create(program = llb , name = 'Section A', year = 1)
        Section.objects.update_or_create(program = llb , name = 'Section B', year = 1 )

        for year in range(2,6):
            Section.objects.update_or_create(program = ballb , name = 'Section A' , year = year)
        for year in range(2,4):
            Section.objects.update_or_create(program = llb , name = 'Section A', year = year)
        ...
        for name , is_law , semester in ballb_subjects:
            Subject.objects.update_or_create(name = name , is_law_subject = is_law , semester = semester)
...
class Command(BaseCommand):
        ...
        for year in range(1, 6):
            if year == 1:
                ballb_sections.extend([
                    Section.objects.update_or_create(program=ballb, name="Section A", year=year)[0],
                    Section.objects.update_or_create(program=ballb, name="Section B", year=year)[0],
                ])
            else:
                ballb_sections.append(Section.objects.update_or_create(program=ballb, name="Section A", year=year)[0])
        for year in range(1, 4):
            if year == 1:
                llb_sections.extend([
                    Section.objects.update_or_create(program=llb, name="Section A", year=year)[0],
                    Section.objects.update_or_create(program=llb, name="Section B", year=year)[0],
                ])
            else:
                llb_sections.append(Section.objects.update_or_create(program=llb, name="Section A", year=year)[0])
    ...
    #             user.save()
    #         Teacher.objects.update_or_create(user = user , first_name = fname, last_name = lname , email = email  )
        ...
        students = [
            ...
            ("G240011", "Harsh", "Verma", "harshu.verma@example.com", "9876543220", ballb_sections[0]),
            ...
            ("G240018", "Ishaan", "Menon", "ishaana.menon@example.com", "9876543227", ballb_sections[1]),
            ...
            ("NG240009", "Harsh", "Verma", "harshs.verma@example.com", "9876543238", llb_sections[0]),
            ...
            ("NG240016", "Ishaan", "Menon", "ishaanw.menon@example.com", "9876543245", llb_sections[1]),
